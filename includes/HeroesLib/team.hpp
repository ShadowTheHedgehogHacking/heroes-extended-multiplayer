...
struct TObjTeam
{
	char GetPlayerNo(const int a1);
	char GetLeaderPlayerNo();
	char GetNowLeaderPlayerNo();
	void SetMutekiTime(unsigned int time);
	void SetItemFlags(int flags);
	bool IsThisPostedPlayer(int playerNo);
	RwV3d* GetCameraTargetPosition();
	void SetCameraTargetPositionHistory(RwV3d* targetPosition);
	...
	short MutekiTime;
	short prohibitionOfFormationChange;
	short ItemFlags;
	char speedLevel;
	char flyLevel;
	char powerLevel;
...
// Global Variables //
extern int* gFormationPositionDat;
