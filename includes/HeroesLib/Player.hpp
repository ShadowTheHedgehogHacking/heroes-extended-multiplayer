...
struct PlayerMaster
{
	...
	static RwV3d* GetPlayerPositionHistory(int pno, unsigned char frame);
	static void SetPlayerPosition(int pno, unsigned char frame, RwV3d* pos, sAngle* ang);
	static void SetPositionP(unsigned char pno, float x, float y, float z);


	static pos_history_struct*& pos_history;
	RwV3d real_pos_history[256][8];
	static ang_history_struct*& ang_history;
	static unsigned char* n_history;
};
//


// Function Prototypes //
signed int GetPlayerPosition(int pno, unsigned char frame, RwV3d* pos, sAngle* ang);
...
extern PlayerMaster playermaster;
// ~Global Variables~ //